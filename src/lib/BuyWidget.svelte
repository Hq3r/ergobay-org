<script lang="ts">
    ...
    async function buy(cancelOrder = false) {
        ...
        const unsigned = await buyTx(box, myAddress, utxos, height, priceInNanoErg, sellerAddress, devAddress, cancelOrder);
...
<div class="p-4 box w-full flex flex-wrap backdrop-blur-md bg-opacity-10 shadow-sm">
    ...
    <div class="flex flex-col justify-end" style="min-width:180px">
        ...
        <button class="btn-secondary p-4" on:click={() => buy(false)}>
            Buy
        </button>
        <button class="btn-secondary p-4" on:click={() => buy(true)}>
            Cancel
        </button>
    </div>
</div>
